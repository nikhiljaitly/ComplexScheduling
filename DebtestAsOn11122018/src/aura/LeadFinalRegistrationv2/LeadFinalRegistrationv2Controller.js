({  // This function that reads the url parameter Lead Id
    ...
    updateLeadJs: function(component, event, helper) {
        ...
        console.log(bankDepositDocAdded);
        if(validityChecker.length > 0 || publicLiabDateValError || workCoverDateValError || policeCheckDocAdded == false || otherInsuranceDocAdded == false || publicLiabilityDocAdded == false || workCoverDocAdded == false || bankDepositDocAdded == false || previousIssuesAdded == false || previousComplaintsAdded == false){
        ...
        action.setCallback(this, function(response) {
            ...
            if(component.isValid() && state == "SUCCESS"){
                ...
                window.location.href = '/t/s/thankyou?FormName=final' ;
            } else {
                console.log('There was a problem : '+response.getError());
            }
        });
